...
### ⚠️ 2. **Challenges in Log Analysis 
...
4. **Limited Analysis Capability:** Humans can’t manually review millions of logs.
    
5. **Format Issues:** Logs come in multiple formats (Windows Event, syslog, Apache, etc.).
    

 **Solution:** Centralized systems that aggregate, normalize, and correlate logs — i.e., **SIEM**.

### 🛡️ 3. **SIEM (Security Information and Event Management)**
...
- User **Haris** logs in from an unknown IP via VPN → accesses sensitive documents → runs PowerShell → outbound connection.  
    Individually benign, but **correlated**, it indicates **possible data exfiltration**.


### 🖥️ 4. **Common Log Sources**
...
### ⚙️ 6. **Detection Rules — The Logic Behind Alerts**
...
Once SIEM triggers an alert, analysts investigate it to confirm if it’s a **true positive (real attack)** or **false positive**.
...
- All authentication logs are stored in the **Security.evtx** log file.
...
- **Default Local System Accounts:**
    ...
    - **SYSTEM:** The most powerful account with complete control over the system. Used by high-privilege services.
        
    - **NETWORK SERVICE:** A limited-privilege account that can authenticate to remote servers using the computer's credentials.
        
    - **LOCAL SERVICE:** A limited-privilege account for local services that does not authenticate over the network.
        
    - **COMPUTERNAME$:** The computer account created when a machine joins a domain. Used for domain authentication.
        
    - **ANONYMOUS LOGON:** Used for network communications that do not require explicit credentials.
...
- **Logon Information > Logon Type:** A numeric code indicating _how_ the user logged on. Crucial for understanding the context.
    
    - **2:** Interactive logon (at the keyboard).
        
    - **3:** Network logon (e.g., accessing a file share over SMB).
        
    - **10:** Remote interactive logon (RDP).
        
- **New Logon Section:** Details about the user who logged on.
    
    - **Account Name:** The username.
        
    - **Account Domain:** Helps identify if it's a local or domain account.
        
    - **Logon ID:** A unique identifier for the logon session.
        
- **Network Information Section:** Shows the source of the logon attempt.
    
    - **Source Network Address:** The IP address of the machine where the credentials were entered. Vital for investigating lateral movement.
        

**Use Cases:** Identify compromised account activities, detect RDP logins from unusual workstations or public IPs, find unauthorized network share access, and spot logins outside working hours.
...
- When an admin account logs in, **two events are always recorded together**:
...
This section explains how to use the **Logon ID** field to track a user's activities during a specific session and determine the session's duration.

**Key Concepts:**

- The **Logon ID** is a unique value for each session and is present in most security events, allowing you to correlate all actions performed during that session (e.g., process execution, file access).
    
- To calculate the **length of an interactive session** (e.g., Logon Types 2, 10), you must correlate the logon event with a logoff event.
    
- **Event ID 4634 - "An account was logged off"** indicates the end of a session.
    
- **Process:** Find a **4624** event (logon) and a subsequent **4634** event (logoff) that share the same **Logon ID**. Subtract the timestamps to find the session duration.
    
- For non-interactive logons (e.g., **Logon Type 3** for network shares), the session starts and ends almost instantly upon accessing the resource, so calculating duration is not useful.

### Tracking Failed Logins & Account Management

#### 1. Tracking Failed Logins (The "Who's Knocking?" Log)

- **Event ID to Hunt:** **4625 - "An account failed to log on."**
    
- **Why It Matters:** This is your primary alert for password attacks. Attackers constantly try to guess passwords to break in.
    
- **The "Failure Info" Section is Key:**
    
    - It tells you _why_ the login failed.
        
    - **`0xC0000064`:** "User doesn't exist." (Attacker is guessing usernames)
        
    - **`0xC000006A`:** "Wrong password." (Attacker has the right user, wrong pass)
        
    - **`0xC0000234`:** "Account is locked out." (Too many wrong tries!)
        
- **Simple Attack Detection:**
    
    - **Brute-Force:** Many failed logins for **one user**.
        
    - **Password Spraying:** A few failed logins for **many different users**.
        
- **The "Lockout" Event:** After too many failures, Windows locks the account and logs **Event ID 4740 - "A user account was locked out."** This is a big red flag.
    
---
#### 2. Login Validation Events (The "Bouncer's" Log)

- **The Big Idea:** While logon events (4624, 4625) are on the computer the user tried to access, **validation events are on the server that checked the password** (like a Domain Controller).
    
- **For NTLM Logins:**
    
    - **Event ID: 4776** - Tracks both successful and failed credential checks.
        
    - The "Error Code" here means the same thing as the "Sub Status" in Event 4625.
        
- **For Kerberos Logins:**
    
    - **Failure Event ID: 4771** - "Kerberos pre-authentication failed."
        
    - It has its own set of "Failure Codes" (e.g., `0x18` means "Wrong password").
...
- **The Most Important Event:** **4720 - "A user account was created."**
    
- **What to Look For:**
    
    - **Subject:** Who created the account? (e.g., `SOC\administrator`)
        
    - **New Account:** What is the name of the newly created account? (e.g., `SOC\HackerAccount`)
...
- **Other Key Management Events:**
    ...
    - **4738:** User account was **changed** (e.g., password reset).
        
#### B. Tracking Creation and Account Adding to Security Groups

- **The "Oh No!" Event:** **4728 - "A member was added to a security-enabled global group."**
    
- **Why It's Critical:** This tells you the **privileges** of a new account. An account is harmless until it's added to a powerful group.
    
- **The Golden Trio of Information:**
    
    1. **Who did it?** (Subject - e.g., `SOC\Administrator`)
        
    2. **Which user was added?** (Member - e.g., `SOC\HackerAccount`)
        
    3. **To which powerful group?** (Group - e.g., `Domain Admins`)
        
- **The Attacker's Playbook:** An attacker's goal is often: **Create Account (4720)** -> **Add to Admin Group (4728)** = Total Control.
